...
const Graph = ({ width, height, graphData, xLeft, xRight, yTop, yBottom }: GraphProps) => {
    ...
    <>
      ...
      <LineChart className='chart-container' width={width} height={height} data={lineData}>
        ...
        <XAxis 
          ...
          ticks={[0, 0.042, 0.083, 0.125, 0.167, 0.208, 0.25, 0.292, 0.333, 0.375, 0.417, 0.458, 0.5, 0.542, 0.583, 0.625, 0.667, 0.708, 0.75, 0.792, 0.833, 0.875, 0.917, 0.958, 1].filter(tick => tick >= xLeft && tick <= xRight)}
          tickFormatter={(tick) => Math.round(tick * 24).toString()}
          type="number" dataKey="x" domain={[xLeft, xRight]} />
        <YAxis width={100} 
          allowDataOverflow={true}
          ticks={graphData.stationSeries.map(s => s.relativePosition).filter(tick => tick >= 1 - yBottom && tick <= 1 - yTop)}
          tickFormatter={(tick) => graphData.stationSeries.find(s => s.relativePosition === tick)!.identifier}
          domain={[1 - yBottom, 1 - yTop]} 
          />
        {graphData.lines.map(
          (line, index) => <Line 
